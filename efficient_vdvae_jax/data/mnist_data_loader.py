    ...
    if hparams.synthesis.synthesis_mode == "reconstruction":
        ...
        test_data = test_data.interleave(
            lambda x: tf.data.Dataset.from_tensor_slices(
                tensors=(x[tf.newaxis, ...], x[tf.newaxis, ...])
            ),
...
def data_prep(img, use_tf=True, return_targets=True):
    ...
    if use_tf:
        ...
        noise = tf.random.uniform(
            shape=img.shape, minval=0.0, maxval=1.0, dtype=img.dtype
        )
        ...
        noise = np.random.uniform(low=0.0, high=1.0, size=img.shape).astype(img.dtype)
...
def download_mnist_datasets():
    ...
    x_train = np.pad(
        x_train, pad_width=((0, 0), (2, 2), (2, 2), (0, 0))
    )  # (60000, 32, 32, 1)
    x_test = np.pad(
        x_test, pad_width=((0, 0), (2, 2), (2, 2), (0, 0))
    )  # (60000, 32, 32, 1)
