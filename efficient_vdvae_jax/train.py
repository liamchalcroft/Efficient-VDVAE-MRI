...
def main():
    ...
    flat_counts, _ = ravel_pytree(
        jax.tree_map(lambda w: jnp.prod(jnp.asarray(w.shape)), init_params)
    )
    n_params = jnp.sum(flat_counts) / 1000000
    print(f"\t\tModel trainable parameters: {n_params:.3f}m")

    print("\tCreating schedule...")
    learning_rate_schedule = get_lr_schedule(
    ...
    state = load_checkpoint_if_exists(
        checkpoint_dir,
        state,
        replace_params_with_emaparams=hparams.train.resume_from_ema,
    )

    print(
        "Beginning training (first step does XLA compilation and can take several minutes)..."
    )
    # Training loop
    train(
        rng=rng,
