...
class KLDivergenceStats(KLDivergence):
    ...
    def compute_loss(self, p, q, global_batch_size):
        raise NotImplementedError(
            "Don't use this class to compute loss! Call compute_metrics instead!!"
        )
    ...
    def _compute_metrics(self, p, q, global_batch_size):
        ...
        # [n_variates, ]
        avg_loss = jnp.sum(avg_per_example_loss, axis=0) / (
            global_batch_size * jnp.log(2.0)
        )  # divide by ln(2) to convert to KL rate (average space bits/dim)
        return avg_loss

    def compute_metrics(
        self, posterior_dist_list, prior_kl_dist_list, global_batch_size
    ):
        ...
        metrics = {
            "per_variate_avg_divs": lax.psum(per_variate_avg_divs, axis_name="shards")
        }
        return metrics
