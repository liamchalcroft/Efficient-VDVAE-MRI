...
def get_lr_schedule(
    decay_scheme, init_lr, warmup_steps, decay_steps, decay_rate, decay_start, min_lr
):
    if decay_scheme == "noam":
        return NoamSchedule(init_lr, warmup_steps=warmup_steps)

    elif decay_scheme == "exponential":
        return NarrowExponentialDecay(
...
class LogisticBetaSchedule:
    ...
    def __call__(self, step):
        return self.beta_max / (
            1.0 + jnp.exp(-self.growth_rate * (step - self.activation_step))
        )
...
class LinearBetaSchedule:
    ...
    def __call__(self, step):
        return jnp.clip(
            (step - self.anneal_start) / (self.anneal_start + self.anneal_steps),
            a_min=self.beta_min,
            a_max=self.beta_max,
        )
...
class GammaSchedule:
    def __init__(self, max_steps):
        ...
        self.num_groups = sum(hparams.model.down_n_blocks_per_res) + len(
            hparams.model.down_strides
        )

    def __call__(self, kl_losses, resolutions, step_n, epsilon=0.0):
        ...
        kl_loss = jax.lax.cond(
            step_n <= self.max_steps, schedule, do_nothing, operand=kl_losses
        )
...
class ConstantLearningRate:
    ...
    def __call__(self, step):
        return self.init_lr * jnp.minimum(1.0, step / self.warmup_steps)
...
class NarrowExponentialDecay:
    ...
    def __call__(self, step):
        return jnp.clip(
            ...
            ^ (step - self.decay_start / self.decay_steps),
            a_min=self.minimum_learning_rate,
            a_max=self.initial_learning_rate,
        )
...
class NarrowCosineDecay:
    ...
    def __call__(self, step):
        ...
        def do_nothing(step_):
            return self.initial_learning_rate * jnp.minimum(
