...
class Loss:
    def __init__(self):
        ...
        if hparams.loss.variation_schedule == "None":
            self.kldiv_schedule = lambda x: 1.0
    ...
    def compute_loss(
        ...
        if variate_masks is None:
            ...
            assert (
                len(variate_masks)
                == len(posterior_dist_list)
                == len(prior_kl_dist_list)
            )

        recon_loss = self.reconstruction_loss.compute_loss(
            targets=targets, logits=logits, global_batch_size=global_batch_size
        )
        ...
        def get_kl_list(p_dist, q_dist, variate_mask):
            ...
            for p, q, vm in zip(p_dist, q_dist, variate_mask):
                kl_loss, resolution = self.kldiv_loss.compute_loss(
                    p=p, q=q, variate_mask=vm, global_batch_size=global_batch_size
                )
        ...
        kl_losses, resolutions = get_kl_list(
            posterior_dist_list, prior_kl_dist_list, variate_masks
        )

        if hparams.loss.use_gamma_schedule:
            kl_div = self.gamma_schedule(kl_losses, resolutions, step_n=step_n)
        else:
    ...
    def compute_metrics(
        ...
        metrics = {
            ...
            "kl_div": lax.psum(
                kl_div / jnp.log(2.0), axis_name="shards"
            ),  # divide by ln(2) to convert to bits/dim (logging only)
            "means": means,
            "log_stds": log_stds,
        }
...
class ReconstructionLayer:
    ...
    def _compute_scales(self, logits):
        if hparams.model.output_distribution_base == "std":
            scales = jnp.maximum(
                beta_softplus(
                    logits, beta=hparams.model.output_gradient_smoothing_beta
                ),
    ...
    def _sample_from_mol(self, key, logits):
        ...
        lambda_ = jnn.one_hot(
            jnp.argmax(logit_probs, axis=-1),
            num_classes=logit_probs.shape[-1],
            dtype=logit_probs.dtype,
            axis=-1,
        )  # B, H, W, M
        ...
        x = means + scales * hparams.synthesis.output_temperature * (
            jnp.log(u) - jnp.log(1.0 - u)
        )  # B, H, W, C

        # Autoregressively predict RGB
        x0 = jnp.clip(
            x[..., 0:1], a_min=self.min_pix_value, a_max=self.max_pix_value
        )  # B, H, W, 1
        x1 = jnp.clip(
            x[..., 1:2] + coeffs[..., 0:1] * x0,
            a_min=self.min_pix_value,
            a_max=self.max_pix_value,
        )  # B, H, W, 1
        x2 = jnp.clip(
    ...
    def _compute_inv_stdv(self, logits):
        if hparams.model.output_distribution_base == "std":
            ...
            inv_stdv = 1.0 / scales  # Not stable for sharp distributions
    ...
    def _get_mol_negative_log_probs(self, targets, logits):
        ...
        inv_stdv, log_scales = self._compute_inv_stdv(
            l[
                :,
                :,
                :,
        ...
        mean2 = (
            model_means[..., 1:2] + model_coeffs[..., 0:1] * targets[..., 0:1]
        )  # B, H, W, M, 1
        mean3 = (
            model_means[..., 2:3]
            + model_coeffs[..., 1:2] * targets[..., 0:1]
            + model_coeffs[..., 2:3] * targets[..., 1:2]
        )  # B, H, W, M, 1
        ...
        log_cdf_plus = plus_in - nn.softplus(
            plus_in
        )  # log probability for edge case of 0 (before scaling)
        log_one_minus_cdf_min = -nn.softplus(
            min_in
        ...
        log_pdf_mid = mid_in - log_scales - 2.0 * nn.softplus(mid_in)

        # Treat edge pixels (0 and 255 before scaling) differently from the middle range.
        broadcast_targets = jnp.broadcast_to(
            targets,
            shape=[
                targets.shape[0],
                targets.shape[1],
        ...
        log_probs = jnp.where(
            broadcast_targets == self.min_pix_value,
            log_cdf_plus,
            jnp.where(
                broadcast_targets == self.max_pix_value,
                log_one_minus_cdf_min,
                jnp.where(
                    cdf_delta > 1e-5,
                    jnp.log(
                        jnp.maximum(cdf_delta, 1e-12)
                    ),  # 1e-12 is ugly trick to get around backprop through jnp.where NaNs
                    log_pdf_mid - jnp.log(self.num_classes / 2),
                ),
            ),
        )  # B, H, W, M, C

        log_probs = jnp.sum(log_probs, axis=-1) + nn.log_softmax(
            logit_probs, axis=-1
        )  # B, H, W, M
    ...
    def _get_bernoulli_log_probs(self, targets, logits):
        ...
        negative_log_probs = jnp.squeeze(
    ...
    def _get_negative_log_probs(self, targets, logits):
        return (
            self._get_bernoulli_log_probs(targets, logits)
            if hparams.data.dataset_source == "binarized_mnist"
            else self._get_mol_negative_log_probs(targets, logits)
        )
    ...
    def compute_loss(self, targets, logits, global_batch_size):
        ...
        loss = jnp.sum(per_example_loss) / (
            global_batch_size * get_effective_n_pixels()
        )

        return loss

    def compute_metrics(self, targets, logits, global_batch_size):
        negative_log_probs, means, log_stds = self._get_negative_log_probs(
            targets, logits
        )

        mean_axis = (1, 2)
        avg_per_example_loss = jnp.sum(negative_log_probs, axis=mean_axis) / (
            jnp.prod(jnp.asarray([negative_log_probs.shape[i] for i in mean_axis]))
            * hparams.data.channels
        )  # B

        assert len(avg_per_example_loss.shape) == 1

        avg_loss = jnp.sum(avg_per_example_loss) / (
            global_batch_size * jnp.log(2.0)
        )  # divide by ln(2) to convert to bits/dim (for visualization purposes only)
...
class KLDivergence:
    def _gauss_logstd_based_kl(self, p, q):
        ...
        inv_q_std = jnp.exp(
            -hparams.model.gradient_smoothing_beta * q_logstd
        )  # 1 / q_std
    ...
    def _gauss_std_based_kl(self, p, q):
        ...
        # KL = 0.5 [(p_mu - q_mu) ** 2 / q_sigma ** 2 + p_sigma ** 2 / q_sigma ** 2 - 1 - 2 log(p_sigma / q_sigma)]
        loss = (
            0.5 * (term1 + term2) - 0.5 - jnp.log(p_std_on_q_std)
        )  # batch, h, w, n_gauss
        return loss
    ...
    def compute_loss(self, p, q, variate_mask, global_batch_size):
        ...
        n_mean_elems = jnp.prod(
            jnp.asarray([loss.shape[a] for a in mean_axis])
        )  # heads * h * w  or h * w * n_variates
