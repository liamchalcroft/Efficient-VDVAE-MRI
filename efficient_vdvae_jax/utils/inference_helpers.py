...
try:
    ...
    from .train_helpers import (
        sample_outputs_from_logits,
        reshape_to_single_device,
        change_unit_of_metrics,
    )
...
def forward_fn(
    params, rng, model, inputs, targets, global_batch_size, training, variate_masks
):
    loss_ = Loss()
    ...
    def metrics_fn(*args, **kwargs):
        return loss_.compute_metrics(
            *args, **kwargs, global_batch_size=global_batch_size
        )

    logits, posterior_dist_list, prior_kl_dist_list = model.apply(
        {"params": params}, rng, inputs, training, variate_masks
    )
    loss, kl_div = loss_fn(
        targets,
        logits,
        posterior_dist_list,
        prior_kl_dist_list,
        max(hparams.loss.vae_beta_anneal_steps, hparams.loss.gamma_max_steps) * 10.0,
        variate_masks,
    )

    metrics = metrics_fn(
        targets, logits, posterior_dist_list, prior_kl_dist_list, kl_div, variate_masks
    )
...
def reshape_distribution(dist_list, variate_masks):
    ...
    def reshape(dist, variate_mask):
        ...
        dist = dist[
            :, :, :, :, variate_mask
        ]  # [2, batch_size, H, W, n_variates_subset]

        dist = jnp.transpose(
            dist, axes=[1, 2, 3, 4, 0]
        )  # [batch_size, H, W, n_variates_subset, 2]
    ...
    dist_list = {
        i: reshape(dist, variate_mask)
        for i, (dist, variate_mask) in enumerate(zip(dist_list, variate_masks))
        if variate_mask.any()
    }
    return dist_list
...
def filter_z(zs, variate_masks):
    ...
    # If the mask states all variates of a layer are not effective, we don't collect any latents from that layer.
    zs = {
        i: _filter(z, variate_mask)
        for i, (z, variate_mask) in enumerate(zip(zs, variate_masks))
        if variate_mask.any()
    }
...
def compute_stats_step(params, rng, inputs):
    ...
    def metrics_fn(*args, **kwargs):
        return stats_.compute_metrics(
            *args, **kwargs, global_batch_size=hparams.synthesis.batch_size
        )

    _, posterior_dist_list, prior_kl_dist_list = model.apply(
        {"params": params}, rng, inputs, False
    )
...
def encode_step(params, rng, inputs, variate_masks):
    ...
    logits, posterior_dist_list, prior_kl_dist_list = model.apply(
        {"params": params}, rng, inputs, False, variate_masks=variate_masks
    )

    posterior_dist_list = reshape_distribution(
        posterior_dist_list, variate_masks=variate_masks
    )  # n_layers_subset x [batch_size, H, W, n_variates_subset, 2]
...
def _reconstruction_mode(
    ...
    artifacts_folder = artifacts_folder.replace(
        "synthesis-images", "synthesis-images/reconstructed"
    )
    ...
    if hparams.synthesis.mask_reconstruction:
        if not os.path.isfile(os.path.join(latents_folder, "div_stats.npy")):
            raise FileNotFoundError(
                "Run div_stats mode before trying to do masked reconstruction!"
            )
    ...
    for step, (inputs, targets) in enumerate(test_dataset):
        ...
        outputs, metrics = p_reconstruction_step(
            params, recon_step_rng, inputs, targets
        )
        metrics["kl_div"].block_until_ready()
        step_time = time() - start_time

        # Compute metrics we care about
        targets, outputs = reshape_to_single_device(
            inputs_tree=(targets, outputs),
        ...
        nelbo = kl + recon
        ssim_per_batch = ssim_metric(
            targets, outputs, global_batch_size=hparams.synthesis.batch_size
        )
        ...
        print(
            f"Step: {step:04d} | Time/Step (sec): {step_time:.4f} | NELBO: {nelbo:.4f} | SSIM: {ssim_per_batch:.4f}    ",
            end="\r",
        )
...
def _generation_mode(
    rng, params, devices, denormalizer, artifacts_folder, latents_folder
):
    artifacts_folder = artifacts_folder.replace(
        "synthesis-images", "synthesis-images/generated"
    )
    os.makedirs(artifacts_folder, exist_ok=True)
    ...
    for temp_i, temperature_setting in enumerate(
        hparams.synthesis.temperature_settings
    ):
        ...
        if isinstance(temperature_setting, list):
            ...
            temp_fn = getattr(temperature_functions, temperature_setting[0])(
                ...
                temperature_setting[2],
                n_layers=len(hparams.model.down_strides),
            )
            temperatures = [
                temp_fn(layer_i) for layer_i in range(len(hparams.model.down_strides))
            ]

        else:
            raise ValueError(
                f"Temperature Setting {temperature_setting} not interpretable!!"
            )
        # Replicate temperatures across devices
        temperatures = jax_utils.replicate(
            jnp.array(temperatures, dtype=jnp.float32), devices=devices
        )
        ...
        for step in range(hparams.synthesis.n_generation_batches):
            ...
            outputs = reshape_to_single_device(
                inputs_tree=outputs, global_batch_size=hparams.synthesis.batch_size
            )  # [batch_size, H, W, C]
            ...
            for output in outputs:
                write_image_to_disk(
                    os.path.join(
                        artifacts_folder, f"setup-{temp_i:01d}-image-{sample_i:04d}.png"
                    ),
                    output,
                    denormalizer=denormalizer,
                )
            ...
            print(
                f"Step: {step:04d} | Time/Step (sec): {step_time:.4f}      ", end="\r"
            )
        print()


def _compute_per_dimension_divergence_stats(
    rng, train_dataset, params, devices, latents_folder
):
    stats_filepath = os.path.join(latents_folder, "div_stats.npy")

    p_compute_stats_step = jax.pmap(fun=jax.jit(compute_stats_step), axis_name="shards")
...
def _encoding_mode(rng, train_dataset, params, devices, latents_folder):
    ...
    for step, (inputs, filenames) in enumerate(train_dataset):
        ...
        posterior_dist = reshape_to_single_device(
            inputs_tree=posterior_dist, global_batch_size=hparams.synthesis.batch_size
        )
        ...
        posterior_dist = jax.tree_map(
            lambda x: {
                name.decode("utf-8"): xa
                for name, xa in zip(filenames, list(np.array(x)))
            },
            posterior_dist,
        )
        ...
        log_inputs = jnp.reshape(
            inputs, newshape=(hparams.synthesis.batch_size,) + inputs.shape[2:]
        )
    ...
    assert (
        encodings["images"].keys() == encodings["latent_codes"].keys()
    )  # All layers share the same filename keys
    print(f"Encoding finished after {step + 1} iterations!")
    with open(
        os.path.join(latents_folder, f"encodings_seed_{hparams.run.seed}.pkl"), "wb"
    ) as handle:
...
def general_inference(rng, params, artifacts_folder, latents_folder, dataset, mode):
    ...
    denormalizer = Denormalizer()
    ssim_metric = StructureSimilarityIndexMap(
        hparams.data.channels, denormalizer=denormalizer
    )
    ...
    if mode == "reconstruction":
        ...
        _compute_per_dimension_divergence_stats(
            rng, dataset, params, devices, latents_folder
        )
...
def synthesize(rng, params, dataset, logdir):
    ...
    general_inference(
        ...
        latents_folder,
        dataset,
        mode=hparams.synthesis.synthesis_mode,
    )
