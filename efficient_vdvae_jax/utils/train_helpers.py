...
def safe_update(state, grads, global_norm, clip_value):
    ...
    skip_bool = jnp.logical_or(
        global_norm >= clip_value, jnp.isnan(global_norm)
    )  # Increment if bigger than clip value or =NaN
...
def reshape_to_single_device(inputs_tree, global_batch_size):
    ...
    return jax.tree_map(
        lambda x: jnp.reshape(x, newshape=(global_batch_size,) + x.shape[2:]),
        inputs_tree,
    )
...
def forward_fn(params, rng, state, inputs, targets, global_batch_size, training):
    ...
    logits, posterior_dist_list, prior_kl_dist_list = state.apply_fn(
        {"params": params}, rng, inputs, training=training
    )
...
def change_unit_of_metrics(metrics):
    ...
    if hparams.data.dataset_source == "binarized_mnist":
        ...
        metrics["kl_div"] = metrics["kl_div"] * jnp.log(2.0) * get_effective_n_pixels()
        metrics["avg_kl_divs"] = jax.tree_map(
            lambda x: x * jnp.log(2.0) * get_effective_n_pixels(),
            metrics["avg_kl_divs"],
        )
        metrics["avg_recon_loss"] = (
            metrics["avg_recon_loss"] * jnp.log(2.0) * get_effective_n_pixels()
        )
    return metrics
...
def train_step(rng, state, train_inputs, train_targets, skip_counter):
    ...
    (_, (logits, metrics)), grads = grad_fn(
        state.params,
        rng,
        state,
        train_inputs,
    ...
    grads, global_norm = clip_by_gradnorm(
        grads, clip_norm=hparams.optimizer.gradient_clip_norm_value
    )

    # Gradient skipping for large gradnorms
    if hparams.optimizer.gradient_skip:
        state, skip_delta = safe_update(
            state, grads, global_norm, hparams.optimizer.gradient_skip_threshold
        )
    else:
...
def eval_step(rng, state, val_inputs, val_targets, batch_size):
    # Forward step
    _, (logits, metrics) = forward_fn(
        state.params, rng, state, val_inputs, val_targets, batch_size, training=False
    )
...
def train(
    ...
    initial_step = int(
        state.step
    )  # get the init step number before replicating the state
    ...
    p_eval_step = jax.pmap(
        fun=jax.jit(
            eval_step, static_argnums=4
        ),  # per_gpu, batch_size is static argument to avoid re-compilation
    ...
    for global_step, (train_inputs, train_targets) in zip(
        range(initial_step, hparams.train.total_train_steps), train_data
    ):
        # Train step
        ...
        state, train_logits, train_metrics, global_norm, skip_counter = p_train_step(
            train_step_rng, state, train_inputs, train_targets, skip_counter
        )

        # Only print once in few steps, this allows asynchronous dispatch to queue multiple successive updates during compute
        if (
            global_step % hparams.train.logging_interval_in_steps == 0
            or global_step % hparams.train.checkpoint_and_eval_interval_in_steps == 0
            or global_step == 0
            or global_step == hparams.train.total_train_steps - 1
        ):
            ...
            n_active_groups = jnp.sum(
                jnp.asarray(
                    [
                        v >= hparams.metrics.latent_active_threshold
                        for v in train_metrics["avg_kl_divs"]
                    ],
                    dtype=jnp.float32,
                )
            )
            print(
                f"global_step: {global_step:08d} | Time/global_step (sec): {step_time:.4f} | "
                ...
                f"Skips: {skip_counter[0]}  ",
                end="\r",
            )

        if (
            global_step % hparams.train.checkpoint_and_eval_interval_in_steps == 0
            or global_step == 0
            or global_step == hparams.train.total_train_steps - 1
        ):
            print()
            ...
            val_losses, val_targets, val_outputs, val_means, val_log_scales = evaluate(
                p_eval_step=p_eval_step,
                rng=val_rng,
                state=state,
                val_data=val_data,
            ...
            train_losses["skips_count"] = (
                skip_counter[0] / hparams.train.total_train_steps
            )  # Log % of cumulative skipped steps
...
def evaluate(p_eval_step, rng, state, val_data, global_step, data_name):
    ...
    for val_step, (val_inputs, val_targets) in zip(
        range(hparams.val.n_samples_for_validation // hparams.val.batch_size), val_data
    ):
        rng, val_sample_key, *val_step_rng = random.split(
            rng, num=jax.local_device_count() + 2
        )
        val_step_rng = jnp.asarray(val_step_rng)
        val_logits, val_batch_metrics = p_eval_step(
            val_step_rng, state, val_inputs, val_targets, batch_size
        )

        val_batch_metrics = jax.tree_map(lambda x: x[0], val_batch_metrics)

        val_targets, val_logits = reshape_to_single_device(
            inputs_tree=(val_targets, val_logits), global_batch_size=batch_size
        )
    ...
    val_losses = {
        "reconstruction_loss": val_metrics["avg_recon_loss"],
    ...
    val_losses.update(
        {f"latent_kl_{i}": v for i, v in enumerate(val_metrics["avg_kl_divs"])}
    )

    print(
        f'Validation Stats for global_step {global_step} on {data_name} | Reconstruction: {val_metrics["avg_recon_loss"]:.6f} | KL: {val_metrics["kl_div"]:.6f} | '
    ...
    return (
        ...
        val_targets,
        val_outputs,
        val_batch_metrics["means"],
        val_batch_metrics["log_stds"],
    )
