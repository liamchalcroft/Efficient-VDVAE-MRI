...
def download_mnist_datasets():
    ...
    x_train = np.pad(
        x_train, pad_width=((0, 0), (0, 0), (2, 2), (2, 2))
    )  # (60000, 1, 32, 32)
    x_test = np.pad(
        x_test, pad_width=((0, 0), (0, 0), (2, 2), (2, 2))
    )  # (60000, 1, 32, 32)
...
transform = transforms.Compose([Binarize()])
...
def train_val_data_mnist(world_size, rank):
    ...
    train_sampler = DistributedSampler(
        train_mnist, num_replicas=world_size, rank=rank, shuffle=True, drop_last=True
    )
    ...
    val_sampler = DistributedSampler(
        val_mnist, num_replicas=world_size, rank=rank, shuffle=True, drop_last=True
    )
