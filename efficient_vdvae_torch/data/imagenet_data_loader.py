...
def read_imagenet_images(path):
    ...
    data = np.concatenate(
        [np.load(f)["data"] for f in files], axis=0
    )  # [samples, C * H * W]
    data = data.reshape(
        [
            data.shape[0],
...
def train_val_data_imagenet(world_size, rank):
    ...
    train_sampler = DistributedSampler(
        train_imagenet, num_replicas=world_size, rank=rank, shuffle=True, drop_last=True
    )
    ...
    val_sampler = DistributedSampler(
        val_imagenet, num_replicas=world_size, rank=rank, shuffle=True, drop_last=True
    )
