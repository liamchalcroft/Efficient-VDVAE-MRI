...
class LogisticBetaSchedule:
    ...
    def __call__(self, step):
        return self.beta_max / (
            1.0 + torch.exp(-self.growth_rate * (step - self.activation_step))
        )
...
class GammaSchedule:
    ...
    def __call__(self, kl_losses, avg_kl_losses, step_n, epsilon=0.0):
...
class ConstantLearningRate(_LRScheduler):
    ...
    def get_lr(self):
        ...
        return [
            v * (torch.minimum(torch.tensor(1.0), self.last_epoch / self.warmup_steps))
    ...
    def _get_closed_form_lr(self):
        return [
            ...
            * (
                torch.minimum(
                    torch.tensor(1.0), torch.tensor(self.last_epoch / self.warmup_steps)
                )
            )
            for v in self.base_lrs
        ]
...
class NarrowExponentialDecay(_LRScheduler):
    def __init__(
        ...
        super(NarrowExponentialDecay, self).__init__(
            optimizer=optimizer, last_epoch=last_epoch, verbose=verbose
        )
    ...
    def get_lr(self):
        lrs = [
            torch.clamp(
                base_lr * self.decay_rate
                ^ (self.last_epoch - self.decay_start / self.decay_steps),
                min=self.minimum_learning_rate,
                max=base_lr,
            )
...
class NarrowCosineDecay(CosineAnnealingLR):
    def __init__(
        ...
        super(NarrowCosineDecay, self).__init__(
            optimizer=optimizer,
    ...
    def get_lr(self):
        if self.last_epoch < self.decay_start:
            ...
            return [
                ...
                * (
                    torch.minimum(
                        torch.tensor(1.0), self.last_epoch / self.warmup_steps
                    )
                )
                for v in self.base_lrs
            ]
...
class NoamSchedule(_LRScheduler):
    ...
    def get_lr(self):
        ...
        return [
            base_lr * self.warmup_steps**0.5 * torch.minimum(arg1, arg2)
            for base_lr in self.base_lrs
        ]
