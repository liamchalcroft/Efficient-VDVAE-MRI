...
class DiscMixLogistic(nn.Module):
    ...
    def forward(self, targets, logits, global_batch_size):
        ...
        # RGB AR
        # print(model_means.shape, model_coeffs.shape, targets.shape)
        mean1 = model_means[:, 0:1, :, :, :]  # B, 1, M, H, W
        mean2 = (
            model_means[:, 1:2, :, :, :]
            + model_coeffs[:, 0:1, :, :, :] * targets[:, 0:1, :, :, :]
        )  # B, 1, M, H, W
        mean3 = (
            model_means[:, 2:3, :, :, :]
        ...
        broadcast_targets = torch.broadcast_to(
            targets, size=[B, C, hparams.model.num_output_mixtures, H, W]
        )
        log_probs = torch.where(
            ...
            torch.where(
                ...
                torch.where(
                    ...
                    log_pdf_mid - np.log(self.num_classes / 2),
                ),
            ),
        )  # B, C, M, H, W

        log_probs = torch.sum(log_probs, dim=1) + F.log_softmax(
            logit_probs, dim=1
        )  # B, M, H, W
        ...
        avg_per_example_loss = per_example_loss / (
            np.prod([negative_log_probs.size()[i] for i in mean_axis])
