...
if hparams.loss.variation_schedule == "None":
    kldiv_schedule = lambda x: torch.as_tensor(1.0)
    ...
    raise NotImplementedError(
        f"KL beta schedule {hparams.loss.variation_schedule} not known!!"
    )
...
def get_optimizer(
    ...
    if decay_scheme == "noam":
        schedule = NoamSchedule(
            optimizer=opt, warmup_steps=warmup_steps, last_epoch=last_epoch
        )

    elif decay_scheme == "exponential":
        schedule = NarrowExponentialDecay(
            optimizer=opt,
        ...
        schedule = ConstantLearningRate(
            optimizer=opt, last_epoch=last_epoch, warmup_steps=warmup_steps
        )
...
def compute_loss(
    ...
    if hparams.data.dataset_source == "binarized_mnist":
        (
            ...
            targets=targets, logits=predictions, global_batch_size=global_batch_size
        )
    else:
        (
            feature_matching_loss,
    ...
    for posterior_dist, prior_kl_dist in zip(posterior_dist_list, prior_kl_dist_list):
        global_variational_prior_loss, avg_global_varprior_loss = kldiv_loss(
            p=posterior_dist, q=prior_kl_dist, global_batch_size=global_batch_size
        )
    ...
    global_variational_prior_losses = torch.stack(
        global_variational_prior_losses, dim=0
    )

    if hparams.loss.use_gamma_schedule:
        global_variational_prior_loss = gamma_schedule(
            global_variational_prior_losses, avg_global_varprior_losses, step_n=step_n
        )
    else:
...
def tensorboard_log(
    ...
    writer.add_histogram(
        ...
        torch.clamp(outputs, min=-1.0, max=1.0),
        global_step,
        bins=20,
    )
    ...
    if mode == "train":
        for variable in model.parameters():
            writer.add_histogram(
                "Weights/{}".format(variable.name), variable, global_step
            )

        # Get the learning rate from the optimizer
        writer.add_scalar(
            "Schedules/learning_rate", optimizer.param_groups[0]["lr"], global_step
        )
...
def _global_norm(model):
    ...
    if len(parameters) == 0:
        ...
        total_norm = torch.norm(
            torch.stack(
                [torch.norm(p.grad.detach(), 2.0).to(device) for p in parameters]
            ),
            2.0,
        )
    return total_norm
...
def gradient_clip(model):
    if hparams.optimizer.clip_gradient_norm:
        total_norm = torch.nn.utils.clip_grad_norm_(
            model.parameters(), max_norm=hparams.optimizer.gradient_clip_norm_value
        )
    else:
...
def gradient_skip(global_norm):
    if hparams.optimizer.gradient_skip:
        if (
            torch.any(torch.isnan(global_norm))
            or global_norm >= hparams.optimizer.gradient_skip_threshold
        ):
            skip = True
            gradient_skip_counter_delta = 1.0

        else:
            skip = False
            gradient_skip_counter_delta = 0.0
...
def _compiled_amp_train_step(model, inputs, step_n, scaler, optimizer):
    with torch.autocast(inputs.device):
        predictions, posterior_dist_list, prior_kl_dist_list = model(inputs)
        (
            avg_feature_matching_loss,
            avg_global_varprior_losses,
...
def reconstruction_step(model, inputs, variates_masks=None, mode="recon"):
    model.eval()
    with torch.no_grad():
        predictions, posterior_dist_list, prior_kl_dist_list = model(
            inputs, variates_masks
        )

        if mode == "recon":
            (
                feature_matching_loss,
                global_varprior_losses,
                total_generator_loss,
                means,
                log_scales,
                kl_div,
                ...
                prior_kl_dist_list=prior_kl_dist_list,
                step_n=max(
                    hparams.loss.vae_beta_anneal_steps, hparams.loss.gamma_max_steps
                )
                * 10.0,
            ...
            if hparams.data.dataset_source == "binarized_mnist":
                ...
                kl_div = kl_div * np.log(2.0) * effective_pixels()
...
def generation_step(model, temperatures):
    y, prior_zs = model.top_down.sample_from_prior(
        hparams.synthesis.batch_size, temperatures=temperatures
    )
...
def encode_step(model, inputs):
    (
        predictions,
        attn_weights_post_list,
        attn_weights_prior_kl_list,
        _,
        _,
        _,
        _,
...
def train(
    ...
    gradient_skip_counter = 0.0

    model.train()

    # let all processes sync up before starting with a new epoch of training
    total_train_epochs = int(
        np.ceil(hparams.train.total_train_steps / len(train_dataset))
    )
    val_epoch = 0
    for epoch in range(0, total_train_epochs):
        ...
        for batch_n, train_inputs in enumerate(train_dataset):
            ...
            if hparams.data.dataset_source == "binarized_mnist":
                # Convert from bpd to nats for comparison
                train_kl_div = train_kl_div * np.log(2.0) * effective_pixels()

            train_var_loss = np.sum(
                [v.detach().cpu() for v in train_global_varprior_losses]
            )
            ...
            if rank == 0:
                print(
                    ...
                    (
                        "Reconstruction Loss",
                        round(train_feature_matching_loss.detach().cpu().item(), 3),
                    ),
                    ...
                    (
                        "Beta",
                        round(kldiv_schedule(global_step).detach().cpu().item(), 4),
                    ),
                    (
                        "N° active groups",
                        np.sum(
                            [
                                v.detach().cpu()
                                >= hparams.metrics.latent_active_threshold
            ...
            if (
                ...
                train_ssim = ssim_metric(
                    train_inputs,
                    train_outputs,
                    global_batch_size=hparams.train.batch_size
                    if hparams.run.local
                    else hparams.train.batch_size // hparams.run.num_gpus,
                )
                if rank == 0:
                    train_losses = {
                        "reconstruction_loss": train_feature_matching_loss,
                        "kl_div": train_kl_div,
                    ...
                    train_losses.update(
                        {
                            f"latent_kl_{i}": v
                            for i, v in enumerate(train_global_varprior_losses)
                        }
                    )
                ...
                for val_step, val_inputs in enumerate(val_dataset):
                    ...
                    (
                        ...
                        val_global_varprior_loss,
                        val_kl_div,
                        val_means,
                        val_log_scales,
                    ) = eval_step(model, inputs=val_inputs, step_n=global_step)

                    val_ssim_per_batch = ssim_metric(
                        val_inputs,
                        val_outputs,
                        global_batch_size=hparams.val.batch_size
                        if hparams.run.local == 1
                ...
                if hparams.data.dataset_source == "binarized_mnist":
                    # Convert from bpd to nats for comparison
                    val_kl_div = val_kl_div * np.log(2.0) * effective_pixels()

                val_global_varprior_losses = [
                    v / (val_step + 1) for v in val_global_varprior_losses
                ]

                val_varprior_loss = np.sum(
                    [v.detach().cpu() for v in val_global_varprior_losses]
                )
                val_nelbo = val_kl_div + val_feature_matching_loss

                if rank == 0:
                    val_losses = {
                        "reconstruction_loss": val_feature_matching_loss,
                        "kl_div": val_kl_div,
                    ...
                    val_losses.update(
                        {
                            f"latent_kl_{i}": v
                            for i, v in enumerate(val_global_varprior_losses)
                        }
                    )
                    ...
                    print(
                        ...
                        f" KL Div {val_kl_div:.4f} |"
                        f"NELBO {val_nelbo:.6f} |"
                        f"SSIM: {val_ssim:.6f}"
                    )
