...
def _process_one_image(filename, in_path, out_path, resolution, resize_type, repeat_n):
    ...
    if resize_type == "reshape":
        ...
        for idx in range(repeat_n):
            ...
            assert np.asarray(out_image).shape == (resolution, resolution, 3)
            out_image.save(
                os.path.join(out_path, filename.replace(".png", f"_re{idx:02d}.png"))
            )
    else:
        raise ValueError(
            f"Provided resize type value not known!! Please verify: {resize_type}"
        )
...
def preprocess(in_path, out_path, resolution, resize_type, repeat_n):
    ...
    if not os.path.exists(in_path):
        raise ValueError(
            f"Provided Input path doesn't exist, please verify your input: {in_path}"
        )
...
def run():
    ...
    parser.add_argument(
        "--in-path",
        help="Input directory that has all the images of the custom dataset.",
        required=True,
    )
    parser.add_argument(
        "--out-path",
        help="Output directory that will have all the images of the custom dataset, preprocessed.",
        required=True,
    )
    parser.add_argument(
        "--resolution",
        type=int,
        help="Desired resolution of the preprocessed dataset.",
        required=True,
    )
    parser.add_argument(
        "--resize-type",
        help="How to reach the desired resolution. One of (center_crop, random_crop, reshape).",
        required=True,
    )
    parser.add_argument(
        "--repeat-n",
